#!/usr/bin/env bash
# ==================================================================
#  ALPminer -- one-click GUI launcher (macOS)
#
#  Double-click this file in Finder, or run it from a terminal in
#  the code folder. It reuses the existing virtual environment when
#  it is healthy and rebuilds it automatically when it is missing or
#  broken (e.g. after copying the folder to another computer or
#  upgrading Python), then starts the GUI server.
#
#  First time only: the file may need to be made executable with
#      chmod +x start-gui-macos.command
...
# work from this script's own folder (double-clicking starts in $HOME)
cd "$(dirname "$0")" || exit 1
